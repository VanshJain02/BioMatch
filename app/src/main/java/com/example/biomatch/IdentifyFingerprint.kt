...
class IdentifyFingerprint : AppCompatActivity() {

    private val cameraRequestCode = 42
    private lateinit var photoFile: File
    private val TAG = "Identify"
    ...
    lateinit var progressBarPercent: TextView
    var progressValue = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_identify_fingerprint)

        var count=0

        val identify_button = findViewById<Button>(R.id.identify_button)
        score = findViewById<TextView>(R.id.identify_score)
        score_value = findViewById<TextView>(R.id.identify_score_value)
        progressBar = findViewById(R.id.progress_bar)
        progressBarPercent = findViewById(R.id.progress_percent)
        progressBar.max = 1000
        progressBar.progress = 0
        progressBarPercent.text = (0.0).toString()
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode == AppCompatActivity.RESULT_OK && allPermissionGranted()) {
            if (requestCode == cameraRequestCode || requestCode==100) {
                runOnUiThread {
                    progressValue += 100
                    ObjectAnimator.ofInt(progressBar, "progress", progressValue).setDuration(10000)
                        .start()
...
//                        .setDuration(10000).start()
                ...
                findViewById<AppCompatButton>(R.id.identify_button).isEnabled = false
                        ...
                        if (greymap != null) {
                            ...
                            CoroutineScope(Dispatchers.IO).launch {
                                val executionTime = measureTimeMillis {
                                    ...
                                    runOnUiThread {
                                        findViewById<AppCompatButton>(R.id.identify_button).isEnabled =
                                            true
                                    }
...
//                                    }
                        ...
                        runOnUiThread {
                            ...
                            Toast.makeText(baseContext, "Upload Both the Images", Toast.LENGTH_LONG)
                                .show()
                            findViewById<Button>(R.id.identify_button).isEnabled = true
...
//                                    val model = SiamesemodelEnh.newInstance(this@PAGE_Matching)  //Recent and best model of siamese enh with scattering2d with input image(96,96)
                val model = ModelScatDwtharrSiameseV1.newInstance(this@IdentifyFingerprint) //input_image(200,200)
...
//        val bitmap = flipBitmapHorizontally(bitma)
    ...
    fun getFireBaseData(saved_finger: List<FloatArray>) {
        runOnUiThread {
            progressValue = 950
            ObjectAnimator.ofInt(progressBar, "progress", progressValue).setDuration(2000).start()
